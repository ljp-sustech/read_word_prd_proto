...
嵌入现有AI入口｜自动判断文档处理需求｜文档总结｜文档问答｜模板化追问｜伪多轮交互

| **文档版本**  | V1.0 评审版                                    |
| --------- | ------------------------------------------- |
| **适用范围**  | PC端、粤政易端（移动端）智慧读文能力设计                       |
| **产品定位**  | 非独立系统；嵌入现有AI入口的文档理解能力增强模块                   |
| **核心能力**  | 文件上传与解析、自动判断文档处理需求、处理目标不明确时反问确认、全文总结、文档问答、引用溯源、模板化推荐追问    |
...
本次设计的核心不是新增一个独立AI产品，而是在现有AI入口和中枢判断体系中新增“智慧读文”能力分支。用户上传文档或输入文档相关问题后，系统自动判断其文档处理需求，并在判断明确时进入智慧读文智能体，完成文件解析、结构化总结、文档问答和引用溯源；若判断不明确，则通过反问确认让用户补全目标。针对当前算力资源不足、无法支持高级多轮自由问答的问题，方案采用模板化“猜你想问”按钮与上下文拼接机制，实现接近多轮追问的产品体验，同时降低用户Prompt使用门槛。

## 1.5 边界：什么做、什么不做

| **类别** | **说明**                                                                          |
| ------ | ------------------------------------------------------------------------------- |
| 做      | 支持文档上传与解析；支持上传后自动判断文档处理需求；支持处理目标不明确时反问确认；支持全文总结、文档问答、引用溯源；支持模板化推荐追问；PC端支持结果流转至已有编辑/润色能力。 |
...
2. 系统在后台自动判断用户的文档处理需求，识别其是否属于“智慧读文”场景，以及更适合进入全文总结、文档问答或解析后问答等流程。
3. 若判断明确，则进入智慧读文智能体；若判断不明确，则触发反问确认，由用户选择“全文总结”或“文档问答”等可见操作。
...
在现有中枢判断体系中新增“智慧读文”分支。这里的“意图识别”不是用户可见的独立功能，而是贯穿文件上传、问题输入和后续追问的后台判断机制。系统负责判断用户当前输入是否属于文档理解任务，并进一步判断应进入全文总结、文档问答、解析后问答或反问确认流程；随后将用户查询词、文件对象、来源端、会话上下文摘要等必要信息透传给智慧读文智能体。

| **用户输入特征**                 | **是否有文件** | **后台判断结果**   | **用户可见处理方式**            |
| -------------------------- | --------- | ---------- | ------------------- |
| 仅上传文件，无文本指令                | 是         | 文档处理目标不明确       | 触发反问确认：全文总结 / 文档问答  |
...
* 触发条件：仅上传文件、用户指令含糊、智慧读文与其他分支置信度接近、文件上传但文档处理目标缺失。
...
| 反问确认后 | 全文总结 / 文档问答            | 用户选择结果作为文档处理目标补全信息          |

## 2.3 与其他智能体协同

### 2.3.1 中枢意图识别智能体协同

中枢意图识别智能体负责在后台判断是否进入“智慧读文”分支，并在文档处理目标不明确时触发反问确认。该能力不作为用户可点击功能单独呈现，而是融入上传文件、输入问题和点击推荐追问后的每一次流程判断。若用户输入属于普通聊天、常识问答、任务交办、下批示或任务问答等明确任务类指令，则保持现有分支逻辑，不抢占。
...
【截图占位-PC-1：PC端入口与助手面板首页】
建议截图内容：现有PC系统页面、右下角“智慧读文”悬浮入口、展开后的智慧读文助手面板、底部快捷能力按钮（全文总结/文档问答/引用溯源）。用于说明智慧读文不是独立系统，而是嵌入现有AI入口的能力分支。

【截图占位-PC-2：PC端上传文件与后台需求判断】
建议截图内容：用户上传PDF/Word文件后，助手展示“正在判断文档处理需求，并解析文件内容”的状态，以及文件卡片、解析进度、后台需求判断提示。用于说明“需求判断”是后台机制，不是用户可点击功能。

【截图占位-PC-3：PC端反问确认】
建议截图内容：仅上传文件或指令含糊时，助手弹出“全文总结/文档问答”两个确认选项。用于说明处理目标不明确时的兜底交互。

【截图占位-PC-4：PC端全文总结结果】
建议截图内容：结构化总结卡片，包含“文档主旨、核心观点、关键结论、风险与建议”等层级，以及回答后的“猜你想问”按钮。用于说明总结输出格式和伪多轮追问入口。

【截图占位-PC-5：PC端文档问答与引用溯源】
建议截图内容：用户围绕文档提问后，AI先给直接结论，再给依据说明，并展示页码/段落引用标签。用于说明回答可信度和原文回溯能力。

【截图占位-PC-6：PC端结果流转】
建议截图内容：总结或问答结果流转到“润色、AI改写、内容编辑、复制引用”等已有PC端能力。用于说明PC端复用既有编辑加工组件。
...
【截图占位-移动-1：移动端首页入口与智慧读文能力按钮】
建议截图内容：粤政易移动端AI办公助手首页、输入框上方能力条、“智慧读文”能力按钮、底部输入框。用于说明移动端入口位置和对话式承载方式。

【截图占位-移动-2：移动端上传文件与解析状态】
建议截图内容：用户上传文件后的文件卡片、AI提示“正在判断文档处理需求，并解析文件内容”、需求判断/文件解析/OCR兜底等流程提示。用于说明上传后自动进入后台判断和解析。

【截图占位-移动-3：移动端反问确认】
建议截图内容：仅上传文件但未说明目标时，AI气泡展示“全文总结/文档问答”按钮。用于说明移动端处理目标不明确时的轻量反问。

【截图占位-移动-4：移动端全文总结结果】
建议截图内容：移动端AI气泡中的结构化总结结果、引用来源标签、回答底部横向滑动的推荐追问按钮。用于说明移动端总结展示和追问体验。

【截图占位-移动-5：移动端文档问答与引用溯源】
建议截图内容：移动端文档问答结果，包含直接结论、依据说明、来源页码/段落标签，以及回答后的推荐追问按钮。用于说明移动端问答可信度和引用回溯。
...
| 处理目标不明确    | 仅上传文件或指令含糊  | 检测到您上传了文件，请确认需要进行全文总结还是文档问答。     | 展示反问按钮   |
...
| **问题**                             | **需确认对象**  | **影响范围**        |
| ---------------------------------- | ---------- | --------------- |
| 现有中枢判断/意图识别智能体如何搭建，是否返回置信度及候选分支？        | 大川/算法或平台同事 | 决定文档处理目标不明确时的反问机制如何接入  |
...
| 后台判断     | 智慧读文命中率   | 明确文档总结/问答场景下成功进入智慧读文分支               |
| 兜底机制     | 反问确认成功率 | 文档处理目标不明确时能正确触发反问，并根据用户选择进入对应流程           |
